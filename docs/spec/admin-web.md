...
### 등록/수정/전문 (같은 폼)
- 등록: 약관 · 언어 · 버전(≤20) · 시행일(오늘 이후) · 시행 활성 · 공개 · 전문(리치 에디터 `RichTextEditor`, TipTap). 전문은 **HTML**로 저장 — 굵게/기울임/밑줄/취소선/제목/목록/글자색. 허용 태그는 백엔드 jsoup 화이트리스트와 동일. 예전 평문 등록분은 편집기에 올릴 때 줄바꿈을 `<p>`/`<br>`로 바꾼다. **앱이 HTML 렌더를 지원한 뒤에 배포**(그 전엔 앱에 태그가 글자로 보임). 중복 기준은 `(type, version, locale)`이라 같은 버전을 국문·영문 각각 등록한다.
