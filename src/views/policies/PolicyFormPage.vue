...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await policiesApi.remove(id.value)
...
<template>
  <PageHeader :title="isEdit ? '약관 수정' : '약관 등록'">
    <template v-if="isEdit" #actions>
      <BaseButton variant="secondary" @click="copyAsNew">이 내용으로 새 버전</BaseButton>
      <BaseButton variant="danger" @click="deleteOpen = true">삭제</BaseButton>
    </template>
  </PageHeader>
    ...
    <form v-if="!loading" class="space-y-5" @submit.prevent="submit">
      <div class="grid grid-cols-4 gap-4">
        <FormField label="약관" required>
          <SelectField v-model="form.type" :options="toOptions(policyTypeLabel)" class="w-full" />
        </FormField>
        <FormField label="언어" required hint="같은 버전을 국문·영문 각각 등록">
          <SelectField v-model="form.locale" :options="toOptions(policyLocaleLabel)" class="w-full" />
        </FormField>
        <FormField label="버전" required hint="예) 1.0, 1.1">
          <TextInput v-model="form.version" required :maxlength="20" />
        </FormField>
        <FormField label="시행일" required hint="오늘 이후로 지정">
          <TextInput v-model="form.effectiveAt" type="date" required />
        </FormField>
      </div>
      <div class="flex gap-8">
        <Toggle v-model="form.enabled" label="시행 활성" />
        <Toggle v-model="form.visible" label="앱 약관보기에 공개" />
      </div>
      <FormField label="전문" required>
        <RichTextEditor v-model="form.content" :min-rows="24" />
      </FormField>
  ...
  <ConfirmDialog
    :open="deleteOpen"
    title="약관 버전 삭제"
    :message="`${policyTypeLabel[form.type]} ${policyLocaleLabel[form.locale]} v${form.version}을 삭제합니다.`"
    confirm-label="삭제"
    danger
    :loading="deleting"
